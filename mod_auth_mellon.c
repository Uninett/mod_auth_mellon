...
/* This is a callback passed to lasso, it is invoked whenever lasso
 * emits a log message. We tag the message with the log_domain and 
 * pass it through to the mellon logging.
 *
 * This allows us to capture log messages generated by the SAML
 * library mellon utilizes.
 */
void lasso_log_func(const gchar *log_domain,
                    GLogLevelFlags lasso_log_level,
                    const gchar *message,
                    gpointer user_data)
{
    server_rec *server = (server_rec *)user_data;
    int ap_log_level = APLOG_INFO;

    /* Translate the log level of the lasso message to the
     * Apache log level */
    switch (lasso_log_level) {
    ...
    ap_log_error(NULL, 0, APLOG_MODULE_INDEX,
                 ap_log_level, APR_SUCCESS, server,
                 "(%s) %s",
                 log_domain, message);
}
 ...
 * the shared memory. In addtion it connects lasso logging to our logging
 * so that lasso messages can be captured as well.
 *
 * Parameters:
                          ...
                          apr_pool_t *tmp, server_rec *s)
{
    ...
    /* Connect lasso's logging to our logging via the lasso_log_func
     * callback handler */
    lasso_log_set_handler(G_LOG_LEVEL_MASK, lasso_log_func, (gpointer)s);
